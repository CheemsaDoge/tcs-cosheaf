...
| Research loop claims accepted/promotion/human-review authority | Loop models carry authority notice; attempts cannot write `kb/accepted/`; loop statuses are runtime-only; no verifier/human-review/promotion authority granted | `test_research_loop_max_attempts_enforcement`, `test_research_loop_cannot_add_after_finalize`, `test_research_loop_attempt_loop_id_mismatch` |
| Research loop storage leaks private data or bypasses Git ignore | Loop/attempt storage under `.cosheaf/research-loops/` is ignored by Git; public mode rejects private content; scanner will block unsafe exports | `test_save_and_load_loop`, `test_list_loops` |
