...
def wf_step(
    ...
    action: Annotated[str, typer.Option("--action")] = "memory.search",
    status: Annotated[str, typer.Option("--status")] = "success",
    json_output: Annotated[bool, typer.Option("--json", is_flag=True)] = False,
) -> None:
...
def wf_readiness(
    ...
    console.print(
        f"Readiness for {workflow_id}: not yet assessable from persisted state"
    )
