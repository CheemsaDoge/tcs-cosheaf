...
from cosheaf.librarian.retrieval import rank as librarian_rank

console = Console()
librarian_app = typer.Typer(
    no_args_is_help=True,
    add_completion=False,
    short_help="Deterministic retrieval policy engine.",
)
...
def rank_cmd(
    ...
    issue: Annotated[str, typer.Option("--issue", help="Issue ID")] = "",
    ...
    console.print(
        f"[bold]Librarian rank[/bold] (query: {query!r}) - "
        f"{candidate_count} hot candidates"
    )
    for c in result.candidates:
        console.print(
            f"  [cyan]{c.artifact_id}[/cyan] score={c.score:.3f} temp={c.temperature}"
        )
