...
from cosheaf.orchestrator_fsm.fsm import (
    create_orchestrator,
)
...
fsm_app = typer.Typer(
    ...
    short_help="Durable orchestrator FSM for issue workflows.",
)
...
def fsm_start(
    ...
    if json_output:
        ...
        console.print(
            f"Orchestrator {orch.orch_id} created (state: {orch.state.value})"
        )


@fsm_app.command("transition")
def fsm_transition(
    orch_id: Annotated[str, typer.Argument()],
