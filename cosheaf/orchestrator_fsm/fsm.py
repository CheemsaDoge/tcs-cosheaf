...
def transition_state(
    record: OrchestratorRecord, target_state: FSMState
) -> OrchestratorRecord:
...
def next_action(record: OrchestratorRecord) -> OrchestratorDecision:
    ...
    if current == FSMState.CREATED:
        return OrchestratorDecision(
            next_state=FSMState.PLANNED, recommendation="Plan the task graph"
        )
    if current == FSMState.PLANNED:
        return OrchestratorDecision(
            next_state=FSMState.READY, recommendation="Mark task graph ready"
        )
    if current == FSMState.READY:
        return OrchestratorDecision(
            next_state=FSMState.RUNNING, recommendation="Start local execution"
        )
    if current == FSMState.RUNNING:
        return OrchestratorDecision(
            next_state=FSMState.FINALIZED, recommendation="Finalize the run"
        )
    if current == FSMState.BLOCKED:
        return OrchestratorDecision(
            next_state=FSMState.READY, recommendation="Unblock and retry"
        )
    if current == FSMState.FINALIZED:
        return OrchestratorDecision(
            recommendation="Orchestrator is finalized, no further actions"
        )
    if current == FSMState.FAILED:
        return OrchestratorDecision(
            next_state=FSMState.ABANDONED,
            recommendation="Orchestrator failed, consider abandoning",
        )
